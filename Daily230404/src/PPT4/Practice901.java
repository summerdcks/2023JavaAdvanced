...
public class Practice901 {	//클래스 선언

	public static void main(String[] args) {	//메인함수 선언
		int k36_iNumVal = 2000000000;	//정수형 변수로 선언
			...
			if(k36_sNumVal.substring(k36_i, k36_i+1).equals("0")) {	
				//substring함수로 뽑아온 sNumVal문자열의 i번째 문자가 0일 때
					...
					if(k36_unitX[k36_j].equals("만") || k36_unitX[k36_j].equals("억")) {	//unitX배열의 j번째 변수가 만이거나 억일때
						if (k36_i > 2 && k36_sNumVal.substring(k36_i-3, k36_i).equals("000")) {	//십만, 백만, 천만의 자리가 모두 0일 때
							//i > 2는 OutOfBoundException을 방지하기 위한 것 문자열의 0,1,2번째 자리는 i-3에서 오류가 발생한다
							k36_sNumVoice = k36_sNumVoice + "";		//만단위 출력을 생략한다
						} else {
						k36_sNumVoice = k36_sNumVoice + "" + k36_unitX[k36_j];		//단위를 붙여준다
						}
					} else {
		...
		if(k36_iNumVal == 0) {	//0일 때
			k36_sNumVoice = k36_sNumVoice + k36_units[0];	//0[영]출력
		}
